...
type AdminWalletStandardAccount = {
  address?: string;
  chains?: string[];
  features?: string[];
};

type AdminWalletStandardWallet = {
  name?: string;
  chains?: string[];
...
type AdminWalletStandardSigner = {
...
async function adminGetWalletStandardSigner(expectedTreasuryWallet: string): Promise<AdminWalletStandardSigner> {
...
function AdminTreasuryPanel({
  ...
  async function buildBatchTransactions(rows: AdminBatchPayoutRow[], treasuryWallet: string) {
    ...
    if (!isSol) {
      setBatchSenderProgress("Reading token mint + treasury token account...");
    ...
    for (let index = 0; index < chunks.length; index += 1) {
      ...
      setBatchSenderProgress(`Building transaction ${index + 1}/${chunks.length}...`);
  ...
  async function sendAllWithTreasuryWallet() {
    ...
    try {
      ...
      const transactions = await buildBatchTransactions(payoutRows, access.treasuryWallet);
